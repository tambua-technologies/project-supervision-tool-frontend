...
const LatestReportListItem = ({ item }) => {
  ...
  const period = `${isoDateToHumanReadableDate(
    start
  )} - ${isoDateToHumanReadableDate(end)}`;
      ...
      <div className="list">
        ...
        <div className="list-items">
          {reports.map((item, index) => (
            <LatestReportListItem key={index} item={item} />
          ))}
        </div>
        <div className="list-footer peacock-blue_color">
          <Link to={allReportsUrl}>View All Reports</Link>
        </div>
      </div>
    </div>
  );
